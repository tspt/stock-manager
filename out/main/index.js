...
function requireStreams$1() {
  ...
  var Buffer2 = require$$0$7.Buffer, Transform = require$$1$3.Transform;
...
class SinaDataSource {
  async fetch(code) {
    ...
    return this.parseData(response2.data);
  }
  parseData(data) {
...
function createWindow() {
  const mainWindow = new electron.BrowserWindow({
    ...
    webPreferences: {
      preload: require$$0$4.join(__dirname, "../preload/index.js"),
      sandbox: false,
      // 关键安全配置
      webSecurity: false,
      // 禁用同源策略（仅限开发环境）
  ...
  if (utils$2.is.dev && process.env["ELECTRON_RENDERER_URL"]) {
    ...
    mainWindow.loadFile(require$$0$4.join(__dirname, "../renderer/index.html"));
  }
